...
class Guy():
	Score = 0
	'''indexes of the body parts angles'''
	bodyPos = 0
	changeBodyPos = 0
	...
	def __init__(self, gameDisplay, DISPLAY_WIDTH, DISPLAY_HEIGHT, LEVELZERO, movePerFrame, COLOURS):
		...
		self.guyy = LEVELZERO
		self.col = COLOURS
		self.size = DISPLAY_HEIGHT/10	#size whith all the parts sizes of the guys body are generate
	...
	def wholeBody(self):      # coordinates of the points of the guys body
		Guy.bodyAngles(self)
		self.neck_point = (
		    int(self.guyx + math.cos(math.radians(self.PossiblebodyAngle[self.bodyPos])) * self.size),    #coordinates  of the neck in terms of the hip
		...
		self.knee1_point = (
		    self.guyx + math.cos(math.radians(self.PossiblehipAngle[self.hipPos])) * self.tight, #coordinates of the knee in terms of the hip
		    self.guyy + math.sin(math.radians(self.PossiblehipAngle[self.hipPos]))  * self.tight
		)
		self.knee2_point = (
			self.guyx + math.cos(math.radians(self.PossiblehipAngle1[self.hipPos])) * self.tight,  #second knee
			self.guyy + math.sin(math.radians(self.PossiblehipAngle1[self.hipPos]))  * self.tight
		)

		self.foot1_point = (
			self.knee1_point[0] + math.cos(math.radians(self.PossiblekneeAngle[self.kneePos])) * self.calf, # coordinates of the foot in terms of the knee
			self.knee1_point[1] + math.sin(math.radians(self.PossiblekneeAngle[self.kneePos])) * self.calf
		)
		self.foot2_point = (
			self.knee2_point[0] + math.cos(math.radians(self.PossiblekneeAngle1[self.kneePos])) * self.calf, # second foot
	...
	def	guy_appirance(self):	#DRAWING THE BODY
		Guy.wholeBody(self)
		# lines which with we draw a guy, most are the same when he has or he hasnt the sponge in hand
